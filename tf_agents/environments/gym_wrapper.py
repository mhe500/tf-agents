...
class GymWrapper(py_environment.PyEnvironment):
  ...
  def _step(self, action):
    ...
    # mevangelista: Gym cannot handle a 0-dimensional ndarray as a scalar.
    # For example, this breaks FrozenLake-v0 when it attempts to use the action
    # as a key into a dictionary because ndarray is not a Hashable.
    if type(action) == np.ndarray and np.ndim(action) == 0:
